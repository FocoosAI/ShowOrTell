...
class GFSAM:
    ...
    def set_target(self, img, img_name):
        if self.tar_img is None or self.tar_img_name != img_name:
    ...
    def predict(self):
        ...
        neg_sim_map, neg_mean_sim_map = self.generate_prior(
            tar_feats_sem, ref_feats_sem, 1 - self.ref_masks
        )
        sim_map, mean_sim_map = self.generate_prior(
            tar_feats_sem, ref_feats_sem, self.ref_masks
        )

        # sim_map       =>  S+_max   (focus on most similar region but increase noise)
        # mean_sim_map  =>  S+_mean  (global similarity towards ref. obj.)
        # cross_sim_map =>  S+_mix   (S+_mean * S+_max)
        ...
        cross_sim_map_ = mean_sim_map * sim_map
        cross_sim_map = (cross_sim_map_ - cross_sim_map_.min()) / (
            cross_sim_map_.max() - cross_sim_map_.min() + 1e-6
        )

        neg_mean_sim_map_std = (neg_mean_sim_map - neg_mean_sim_map.min()) / (
            neg_mean_sim_map.max() - neg_mean_sim_map.min() + 1e-6
        )
        neg_region = neg_mean_sim_map_std > cross_sim_map
        mean_sim_map_fil = cross_sim_map * ~neg_region
        coord_xy, coord_labels, sim_map_hot, coord_f = self.find_points(
            mean_sim_map_fil, tar_feats
        )

        tar_masks_list = self.generate_sam_masks(tar_feats, coord_xy, coord_labels)

        if len(tar_masks_list) == 0:
            tar_masks = torch.zeros(
                (1, 1, self.input_size[0], self.input_size[1]), device=self.device
            )
            pred_masks = tar_masks.squeeze(0)
            prob_masks = torch.zeros_like(pred_masks)

            return pred_masks, prob_masks, (coord_xy, None)
        else:
            tar_masks = torch.cat(tar_masks_list, dim=0)

            components_weak, labels_weak, components_strong, labels_strong = (
                self.mask_cluster(tar_masks, coord_f, sim_map_hot)
            )

            fgbg_com_labels, fgbg_labels, pseudo_masks, cls_scores = (
                self.cluster_classification(
                    tar_masks,
                    labels_weak,
                    components_weak,
                    mean_sim_map * mean_sim_map,
                    neg_mean_sim_map * mean_sim_map_half,
                    coord_f,
                )
            )

            selected_points = self.point_consistency_dis(
                tar_feats_sem,
                tar_masks,
                labels_weak,
                components_weak,
            ...
            # mean_sim_map => [1, 37, 37]
            # tar_masks => [nmasks, 1, 518, 518]
            pred_masks, prob_masks = self.triplet_selection_b(
                tar_masks,
                labels_weak,
                selected_points,
                mean_sim_map,
            ...
            # print(cls_scores[selected_points != 2].cpu().numpy())

            return pred_masks, prob_masks, (coord_xy, selected_points)

    def triplet_selection_b(
        self,
        tar_masks,
        cluster_labels,
        coord_se_labels,
        ...
        cluster_labels = torch.as_tensor(
            cluster_labels, device=self.device, dtype=torch.long
        )
        ...
        sim_map_rsz = F.interpolate(
            mean_sim_map.unsqueeze(0),
            self.input_size,
            mode="bilinear",
            align_corners=False,
        ).squeeze(0)
        ...
        if pred_masks.sum() == 0:
            # print("No mask selected, using the most confident mask")
            cls_scores[coord_se_labels != 2] = 0
            _, max_cls_scores_arg = cls_scores.max(dim=0)
            pred_masks += tar_masks[max_cls_scores_arg]
            prob_masks = (
                (sim_map_rsz * tar_masks[max_cls_scores_arg]).sum()
                / tar_masks[max_cls_scores_arg].sum()
                * tar_masks[max_cls_scores_arg]
            )

        return pred_masks, prob_masks

    def cluster_classification(
        self, tar_masks, cluster_labels, n_components, mean_sim_map, neg_map, coord_f
    ):
        ...
        fgbg_com_labels = torch.zeros(
            n_components, device=self.device, dtype=torch.long
        )
        ...
        for component in range(n_components):
            ...
            tar_mask_rsz = F.interpolate(
                tar_masks[com_args].float(),
                (self.encoder_feat_size, self.encoder_feat_size),
                mode="bilinear",
                align_corners=False,
            )
            mask_scores = (tar_mask_rsz * (mean_sim_map - neg_map).unsqueeze(0)).sum(
                dim=(-1, -2, -3)
            )
            mask_scores = mask_scores / (tar_mask_rsz.sum(dim=(-1, -2, -3)) + 1e-6)
            mask_args = mask_scores.argsort(descending=True)
            inner_mask = torch.zeros_like(
                union_mask, device=self.device, dtype=torch.float
            )
            # Mask Growth
    ...
    def point_consistency_dis(
        ...
        for component in range(n_components):
            ...
            _, union_similarity = self.generate_prior(
                tar_feats_sem, tar_feats_sem, union_mask.unsqueeze(0)
            )
        ...
        coord_similarities = union_similarities[
            :, coord_f[:, 1], coord_f[:, 0]
        ]  # nc, nm
        ...
        coord_similarities_max, coord_similarities_max_args = coord_similarities.max(
            dim=0
        )  # nm

        # labeling the points for self consistency
        coord_selection_labels = torch.where(
            coord_similarities_max_args == cluster_labels, 1, 0
        )
        for component in range(n_components):
            com_args = torch.where(cluster_labels == component)[0]
    ...
    def mask_cluster(self, tar_masks, coord_f, sim_map_hot):
        ...
        sim_map_hot = torch.as_tensor(
            sim_map_hot > 0, device=self.device, dtype=torch.float
        )

        tar_masks_rsz = F.interpolate(
            tar_masks.float(),
            (sim_map_hot.shape[-2], sim_map_hot.shape[-1]),
            mode="nearest",
        ).squeeze(1)
        ...
        n_components_strong, labels_strong = csgraph.connected_components(
            adjacent.cpu().numpy(),
            directed=True,
            connection="strong",
            return_labels=True,
        )
        n_components_weak, labels_weak = csgraph.connected_components(
            adjacent.cpu().numpy(), directed=True, connection="weak", return_labels=True
        )
    ...
    def generate_sam_masks(self, tar_feats, coord_xy, coord_labels):
        ...
        for (points,), (labels,) in zip(
            batch_iterator(64, coord_xy), batch_iterator(64, coord_labels)
        ):
    ...
    def generate_prior(self, query_feat_high, supp_feat_high, s_mask):
        ...
        # query_feat_high => [1, c, h*w]
        # supp_feat_high  => [nshot, c, h*w]
        similarities = self.generate_pixelwise_comparison(
            query_feat_high, supp_feat_high
        )
        ...
        for st, supp_feat in enumerate(supp_feat_high):
            ...
            tmp_mask = F.interpolate(
                tmp_mask, size=(sp_sz, sp_sz), mode="bilinear", align_corners=True
            )
            tmp_mask = tmp_mask.flatten(2).transpose(-1, -2)  # [bs, h*w, 1]

            similarity = similarities[st] * tmp_mask  # [bs, h*w, h*w]
            cos_similarity = similarity.mean(1).view(bsize, 1, sp_sz, sp_sz) / (
                tmp_mask.sum() / sp_sz2 + cosine_eps
            )
    ...
    def generate_pixelwise_comparison(self, query_feat_high, supp_feat_high):
        ...
        for st, supp_feat in enumerate(supp_feat_high):
            ...
            similarity = torch.bmm(tmp_supp, tmp_query) / (
                torch.bmm(tmp_supp_norm, tmp_query_norm) + 1e-7
            )  # [bs, h*w, h*w]
    ...
    def extract_img_feats(self):
        ref_imgs = torch.cat(
            [self.encoder_transform(rimg)[None, ...] for rimg in self.ref_imgs], dim=0
        )
        tar_img = torch.cat(
            [self.encoder_transform(timg)[None, ...] for timg in self.tar_img], dim=0
        )

        ref_feats = self.encoder.forward_features(ref_imgs.to(self.device))[
            "x_prenorm"
        ][:, 1:]
        tar_feat = self.encoder.forward_features(tar_img.to(self.device))["x_prenorm"][
            :, 1:
        ]
...
def build_model(args):
    ...
    return GFSAM(encoder=dinov2, generator=predictor, device=args.device)
