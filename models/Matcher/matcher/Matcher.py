...
class Matcher:
    ...
    def predict(self):
        ...
        dino_confidence = S_forward.unsqueeze(0).mean(1).view(1, self.encoder_feat_size, self.encoder_feat_size)
        dino_confidence_rsz = F.interpolate(dino_confidence.unsqueeze(0), self.input_size, mode='bilinear', align_corners=False).squeeze(0)

        curr_prob_mask = (dino_confidence_rsz * pred_masks[0]).sum() / pred_masks[0].sum() * pred_masks[0]

        return pred_masks, curr_prob_mask
    ...
    def mask_generation(self, tar_img_np, points, box, all_ponits, ref_masks_pool, C):
        ...
        # print(f"Predicted masks: {len(tar_masks)}")
        ...
        pred_masks = check_pred_mask(pred_masks)  # N, H, W
        ...
        return torch.tensor(masks, device=self.device, dtype=torch.float), {'all_masks': pred_masks, 'scores': scores} | metric_preds
